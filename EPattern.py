...
class EPattern:
    ...
    def get_size(self):
        gap_size = (self.element_size - 1) * self.pixels_gap
        pixel_size =  self.element_size * self.pixels_size
        return(gap_size + pixel_size)
    ...
    def generate_element(self):
        ...
        for _ in range(self.fillin_pixels_amount):
            ...
            x = _x * (self.pixels_gap + self.pixels_size)
            y = _y * (self.pixels_gap + self.pixels_size)
    ...
    def transform_coord(self, element_index):
        ...
        if element_index == 0: 
            ...
            for x, y in self.pixels_coordinates:
                _x = 2*side - x - self.pixels_gap
                _y = y
                new_coords.append((_x, _y))
        elif element_index == 2:
            for x, y in self.pixels_coordinates:
                _x = 2*side - x - self.pixels_gap
                _y = 2*side - y - self.pixels_gap
                new_coords.append((_x, _y))
        else:
            for x, y in self.pixels_coordinates:
                _x = x
